"use client";

import React from "react";
import Image from "next/image";
import Link from "next/link";
import { motion } from "framer-motion";
import Hero from "@/components/Hero";
import SearchBar from "@/components/SearchBar";
import FeaturedProperties from "@/components/FeaturedProperties";
import { useLanguage } from "@/context/LanguageContext";

const HomePage: React.FC = () => {
  const { t } = useLanguage();

  const locations = [
    {
      name: t("location.sofia"),
      properties: 150,
      image: "https://img.capital.bg/shimg/zx1200y675_3934291.jpg",
    },
    {
      name: t("location.varna"),
      properties: 85,
      image: "https://darikradio.bg/media/356/ttz-varna.m.jpg",
    },
    {
      name: t("location.plovdiv"),
      properties: 65,
      image: "https://img.capital.bg/shimg/zx1200y675_3964872.jpg",
    },
    {
      name: t("location.bansko"),
      properties: 45,
      // Image replaced due to broken path - using high-quality Unsplash image
      image:
        "https://images.unsplash.com/photo-1551524559-8af4e6624178?w=400&h=300&fit=crop",
    },
  ];

  const services = [
    {
      icon: (
        <svg
          className="h-8 w-8"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
          aria-hidden="true"
        >
          <path
            strokeLinecap="round"
            strokeLinejoin="round"
            strokeWidth={2}
            d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"
          />
        </svg>
      ),
      title: "Property Search",
      description:
        "Find your perfect property with our advanced search tools and extensive database.",
    },
    {
      icon: (
        <svg
          className="h-8 w-8"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
          aria-hidden="true"
        >
          <path
            strokeLinecap="round"
            strokeLinejoin="round"
            strokeWidth={2}
            d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"
          />
        </svg>
      ),
      title: "Secure Transactions",
      description:
        "We ensure safe and transparent property transactions with legal support.",
    },
    {
      icon: (
        <svg
          className="h-8 w-8"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
          aria-hidden="true"
        >
          <path
            strokeLinecap="round"
            strokeLinejoin="round"
            strokeWidth={2}
            d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"
          />
        </svg>
      ),
      title: "Expert Consultation",
      description: "Get advice from our experienced real estate professionals.",
    },
    {
      icon: (
        <svg
          className="h-8 w-8"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
          aria-hidden="true"
        >
          <path
            strokeLinecap="round"
            strokeLinejoin="round"
            strokeWidth={2}
            d="M3.055 11H5a2 2 0 012 2v1a2 2 0 002 2 2 2 0 012 2v2.945M8 3.935V5.5A2.5 2.5 0 0010.5 8h.5a2 2 0 012 2 2 2 0 104 0 2 2 0 012-2h1.064M15 20.488V18a2 2 0 012-2h3.064M21 12a9 9 0 11-18 0 9 9 0 0118 0z"
          />
        </svg>
      ),
      title: "International Clients",
      description: "We serve clients worldwide with multi-language support.",
    },
  ];

  return (
    <div className="min-h-screen">
      {/* Hero Section */}
      <Hero />

      {/* Search Bar */}
      <div className="relative z-20">
        <SearchBar />
      </div>

      {/* Locations Section */}
      <section className="bg-white py-20" aria-labelledby="locations-heading">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            className="mb-12 text-center"
          >
            <h2
              id="locations-heading"
              className="mb-4 font-display text-3xl font-bold text-gray-900 md:text-4xl"
            >
              {t("property.location")}s
            </h2>
            <p className="mx-auto max-w-2xl text-gray-600">
              Explore properties in Bulgaria&apos;s most desirable locations
            </p>
          </motion.div>

          <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
            {locations.map((location, index) => (
              <motion.div
                key={location.name}
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                transition={{ delay: index * 0.1 }}
                whileHover={{ y: -5 }}
                className="group relative cursor-pointer overflow-hidden rounded-xl"
              >
                <div className="relative aspect-[4/3]">
                  <Image
                    src={location.image}
                    alt={`${location.name} cityscape`}
                    fill
                    className="object-cover transition-transform duration-300 group-hover:scale-110"
                    sizes="(max-width: 768px) 100vw, (max-width: 1024px) 50vw, 25vw"
                  />
                </div>
                <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/20 to-transparent" />
                <div className="absolute bottom-0 left-0 right-0 p-4">
                  <h3 className="text-lg font-semibold text-white">
                    {location.name}
                  </h3>
                  <p className="text-sm text-white/70">
                    {location.properties} properties
                  </p>
                </div>
              </motion.div>
            ))}
          </div>
        </div>
      </section>

      {/* Services Section */}
      <section className="bg-gray-50 py-20" aria-labelledby="services-heading">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            className="mb-12 text-center"
          >
            <h2
              id="services-heading"
              className="mb-4 font-display text-3xl font-bold text-gray-900 md:text-4xl"
            >
              {t("footer.services")}
            </h2>
            <p className="mx-auto max-w-2xl text-gray-600">
              Comprehensive real estate services to meet all your needs
            </p>
          </motion.div>

          <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-4">
            {services.map((service, index) => (
              <motion.article
                key={service.title}
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                transition={{ delay: index * 0.1 }}
                whileHover={{ y: -5 }}
                className="rounded-xl bg-white p-6 shadow-lg transition-shadow hover:shadow-xl"
              >
                <div
                  className="mb-4 flex h-14 w-14 items-center justify-center rounded-lg bg-primary-100 text-primary-600"
                  aria-hidden="true"
                >
                  {service.icon}
                </div>
                <h3 className="mb-2 text-lg font-semibold text-gray-900">
                  {service.title}
                </h3>
                <p className="text-sm text-gray-600">{service.description}</p>
              </motion.article>
            ))}
          </div>
        </div>
      </section>

      {/* Featured Properties */}
      <FeaturedProperties />

      {/* CTA Section */}
      <section
        className="bg-gradient-to-r from-primary-600 to-primary-800 py-20"
        aria-labelledby="cta-heading"
      >
        <div className="mx-auto max-w-4xl px-4 text-center sm:px-6 lg:px-8">
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
          >
            <h2
              id="cta-heading"
              className="mb-4 font-display text-3xl font-bold text-white md:text-4xl"
            >
              Ready to Find Your Dream Property?
            </h2>
            <p className="mb-8 text-lg text-white/80">
              Contact our expert team today and let us help you find the perfect
              property in Bulgaria.
            </p>
            <div className="flex flex-wrap justify-center gap-4">
              <Link
                href="/contact"
                className="rounded-lg bg-white px-8 py-3 font-semibold text-primary-600 transition-colors hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-primary-600"
              >
                {t("contact.title")}
              </Link>
              <Link
                href="/properties"
                className="rounded-lg border border-white/30 bg-primary-500 px-8 py-3 font-semibold text-white transition-colors hover:bg-primary-400 focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-primary-600"
              >
                {t("properties")}
              </Link>
            </div>
          </motion.div>
        </div>
      </section>
    </div>
  );
};

export default HomePage;
